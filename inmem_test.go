...
//
func TestTTLNotEnforced(t *testing.T) {
	c := inmem.NewUnlocked(2, false)
	e := time.Now().Add(-time.Hour)
	c.Add(1, 1, e)
	ensure.DeepEqual(t, c.Len(), 1)
	_, found := c.Get(1)
